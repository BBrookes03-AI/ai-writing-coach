...
# Configure OpenAI
# You can keep using env var or st.secrets
...
def get_ai_feedback(text: str, writing_type: str) -> str:
    ...
    if citation_data["total_citations"] > 0:
        citation_context = (
            f"\n\nNote: I detected {citation_data['total_citations']} citations in this text. "
            "Consider how well these sources are integrated into the argument."
        )

    full_prompt = (
        f"{prompt_template}{citation_context}\n\n"
        "Keep your response under 300 words, be encouraging but specific, and focus on the most important improvements."
    )

    try:
        resp = client.chat.completions.create(
            model="gpt-4o",  # or "gpt-4.1" / your preferred GPT-4 variant
            messages=[
        ...
        return (
            "I'm having trouble connecting to provide feedback right now. "
...
with col1:
    writing_type = st.selectbox(
        ...
        ["Discussion Post", "Learning Journal", "Academic Paper", "Thesis Statement"],
...
if st.button("Get Nova's Feedback", type="primary"):
    if not user_input.strip():
        ...
        st.error("OpenAI API key not found. Set OPENAI_API_KEY in the environment or st.secrets.")
    else:
